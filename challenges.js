...
function reverseUppercaseString(str) {
  let string = str;
...
function isPalindrome(str) {
  let stringOne = str.toLowerCase().split("");
  for (i = 0; i < stringOne.length; i++) {
    if (stringOne[i] === " ") {
      stringOne.splice(i, 1);
    }
  }
  let newString = stringOne.join("");
  let revString = stringOne.reverse().join("");
  return newString === revString ? true : false;
}
/*-----------------------------------------------------------------
...
function hammingDistance(str, str2) {
  let count = 0;
...
function mumble(str) {
  ...
  for (i = 0; i < str.length; i++) {
    newString += ((i || "") && "-") + str.charAt(i).repeat(i + 1);
  }
  return newString;
}
/*-----------------------------------------------------------------
...
function fromPairs(array) {
  let output = {};
  array.forEach(function (kval) {
    output[kval[0]] = kval[1];
  });
  return output;
}
/*-----------------------------------------------------------------
...
function findHighestPriced(arr) {
  ...
  arr.forEach(function (thing) {
    if (thing.price > highPrice) {
      highPrice = thing.price;
      answer = thing;
    }
  });
  return answer;
...
function mapArray(arr, callback) {
  let newArray = [];
  arr.forEach((ele, idx) => {
    newArray.push(callback(ele, idx));
  });
  return newArray;
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, callback, iacc) {
  let acc = iacc;
  arr.forEach((el, idx) => {
    acc = callback(acc, el, idx);
  });
  return acc;
}
/*-----------------------------------------------------------------
...
// if a number is prime, push it into an array by itself
// if a number is NOT prime, divide it by a prime number then check if the number is prime, if prime, push the divisor and the prime number into an array
// if a number that has been divided by a prime number can still be divided by a prime number, go to the next prime number and see if that has a remainder
...
function intersection(arr1, arr2) {
  ...
  arr1.forEach(val => {
    ...
    if (idx > -1) newArr.push(_arr2.splice(idx, 1)[0]);
  });
  return newArr;
}
/*-----------------------------------------------------------------
...
function balancedBrackets(str) {
  ...
  for(let i=0; i < str.length; i++){ 
    ...
    if (currentChar === '(' || currentChar === '[' || currentChar === '{') {
      ...
      if(currentChar !== map[lastOpenParens]){
        return false;
      }
    }
  }
...
function getNumForIP(ip){
  ...
  j = 0
  strNum = ip.split('.')
  strNum.forEach(function(el){
    intNums.push(parseInt(el))
  })
  for (i = intNums.length - 1; i >= 0; i--){
    num += intNums[i] * (256 ** j)
    j += 1;
  }
  return num
...
function toCamelCase(str) {
  ...
  for (i=0; i < splitString.length; i++) {
    if (splitString[i] === "-" || splitString[i] === "_") {
      let v = splitString[i+1].toUpperCase()
      splitString.splice(i,2,v)
    }
  }
  output = splitString.join("")
  return output
}
/*-----------------------------------------------------------------
...
function addChecker(arg1, arg2) {
  let answer = false
  for(i=0; i<arg1.length; i++){
    for(j=i+1; j<arg1.length; j++){
